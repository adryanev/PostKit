...
private let maxMemorySize: Int64 = 1_000_000
...
actor CurlHTTPClient: HTTPClientProtocol {
    private let performQueue = DispatchQueue(label: "com.postkit.curl-perform", qos: .userInitiated)
    ...
    func execute(_ request: URLRequest, taskID: UUID) async throws -> HTTPResponse {
        ...
        guard !sanitizedURL.contains("\0") else {
            throw HTTPClientError.invalidURL
        }
        ...
        return try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { continuation in
                performQueue.async {
                    ...
                    let setupResult = self.setupHandle(handle, request: request, sanitizedURL: sanitizedURL, context: context, headerList: &headerList)
