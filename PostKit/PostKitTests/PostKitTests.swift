...
struct RequestViewModelTests {
    ...
    @Test func executeRequestReturnsResponse() async throws {
        ...
        let container = try ModelContainer(for: HTTPRequest.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))
    ...
    @Test func newRequestCancelsPrevious() async throws {
        ...
        await viewModel.sendRequest(for: request1)
        await viewModel.sendRequest(for: request2)
    ...
    @Test func buildURLRequestWithQueryParams() throws {
        let container = try ModelContainer(for: HTTPRequest.self, APIEnvironment.self, Variable.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))
        let viewModel = RequestViewModel(modelContext: container.mainContext)
    ...
    @Test func historyCleanupAfterThreshold() async {
        let mockClient = MockHTTPClient(response: HTTPResponse(
            statusCode: 200,
            statusMessage: "OK",
            headers: [:],
        ...
        let container = try! ModelContainer(for: HTTPRequest.self, HistoryEntry.self, configurations: ModelConfiguration(isStoredInMemoryOnly: true))
        let viewModel = RequestViewModel(modelContext: container.mainContext)
